...
GOBIN ?= $(GOPATH)/bin
SUPPLYCHAINLINTER ?= $(GOBIN)/supply-chain-linters
GINKGO ?= $(GOBIN)/ginkgo
...
test: install-test-deps build-race fmt vet govulncheck
	$(GINKGO) -v --fail-fast
...
build:
	go build -o $(BIN) ./cmd/supply-chain-linters/

build-race:
	go build -race -o $(BIN) ./cmd/supply-chain-linters/
...
release:
	@echo "Releasing the supply-chain-linters binary..."
	goreleaser release
...
build-linux:
	CGO_ENABLED=$(CGO_ENABLED) GOOS=linux go build -ldflags=$(BUILDFLAGS) -o $(BIN) ./cmd/supply-chain-linters/
...
.PHONY: test build clean release image image-push tlsconfig install-goreleaser